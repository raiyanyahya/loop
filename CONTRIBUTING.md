...
Bump `version` in `package.json`, add a `CHANGELOG.md` entry, tag `vX.Y.Z`, and push the tag. The Publish workflow runs the tests and publishes `theloop` to npm through trusted publishing (OIDC, no token, provenance automatic); the trusted publisher is configured once in the package's settings on npmjs.com. The Website workflow deploys `docs/` on every push that touches it.
